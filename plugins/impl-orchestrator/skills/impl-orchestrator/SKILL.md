...
5. **検証コマンドを決定**: ビルド設定から正確なコマンドを特定
   ...
   - E2E_TEST: E2E テストコマンドパターン（あれば）
     ...
     - **プロジェクト固有のモードフラグがあれば必ず含める**:
       - 例 (kauche-app): `go test -run {TEST_NAME} -e2emode=coverage ./e2etests/...`
         `-e2emode=coverage` は E2E テスト自身が必要なマイクロサービス（customer, farm 等）を
         内蔵起動するモード。事前の docker サービス起動が不要になる（後述 Step 0.5 と整合）
...
**チェック対象の範囲**:
- ✅ **対象**: emulator (Spanner / Pub/Sub / Datastore / Firestore / Bigtable) と DB (PostgreSQL / Redis / MySQL) と wrench 等の schema ツール — 単体テスト・E2E 双方で必要なため、Step 0.5 で事前確認する
- ❌ **対象外**: E2E テストが **内蔵起動するマイクロサービス**（kauche-app の customer / farm / ec_supplier 等のドメインサービス）
  - 判定: E2E_TEST コマンドに `-e2emode=coverage` 等のモードフラグが含まれていれば「内蔵起動モード」と認識し、対象外にする
  - これらのドメインサービスは E2E 実行時にテスト自身が起動するため、事前の docker 起動確認は不要

**チェック項目**（プロジェクトに該当するものだけ実行）:

1. **Docker コンテナ起動状況**（emulator/DB コンテナが対象。マイクロサービスコンテナは内蔵起動モードでは対象外）:
   - `docker ps --format 'table {{.Names}}\t{{.Ports}}\t{{.Status}}'` で全コンテナ観測
   - 該当 **emulator/DB** のコンテナが見えなければ「起動していない可能性あり」として 2 へ
